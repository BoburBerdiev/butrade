...
const ProductCard = ({product}) => {
  ...
  return (
    <div className={`${isRow ? 'flex-row  w-full' :'flex-col px-2 py-3 lg:px-2.5 lg:py-4 gap-3 lg:gap-4 '} w-full bg-white rounded-lg h-full shadow-md z-[5] hover:rounded-b-none flex   relative group duration-200 hover:shadow-lg`}>
      ...
      <div className={` ${isRow ? 'place-self-end place-content-end justify-self-end' : 'md:grid md:grid-rows-[0fr] z-10 md:group-hover:grid-rows-[1fr] md:absolute top-full duration-200 left-0 right-0 w-full md:bg-white px-2  lg:px-5 md:shadow-lg md:rounded-b-lg'} `}>
