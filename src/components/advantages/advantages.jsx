            ...
            advantagesList?.map(item => (
                <div key={item?.id}
                  ...
                  <div className="relative w-10 h-10 shrink-0  ">
                    <ImageUI src={item?.image} alt={langSelect(i18next.language , item?.title_ru , item?.title_uz)}/>
                  </div>
                  <div className="flex flex-col gap-1 text-start">
                    <h3 className="text-currentBlue text-base md:text-lg font-notoSansDisplay font-semibold"> {langSelect(i18next.language , item?.title_ru , item?.title_uz)}</h3>
                    <p className="font-notoSans text-base ">{langSelect(i18next.language , item?.sub_title_ru , item?.sub_title_uz)} </p>
                  </div>
                </div>
            ))
          }
