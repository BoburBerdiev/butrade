    ...
    <div className={`${isRow ? 'grid grid-cols-7 md:grid-cols-11 gap-x-3 justify-end p-2 md:p-3 lg:p-4 gap-y-3' :'flex-col px-2 py-3 lg:px-2.5 lg:py-4 gap-3 lg:gap-4 hover:rounded-b-none    '} w-full  bg-white rounded-lg h-full shadow-md z-[5]  flex   relative group duration-200 hover:shadow-lg`}>
      ...
      <div className={` ${isRow ? 'w-full col-span-7 md:col-span-3' : 'md:grid md:grid-rows-[0fr] md:§!z-[999] md:group-hover:grid-rows-[1fr] md:absolute top-full duration-200 left-0 right-0 w-full md:bg-white px-2  lg:px-5 md:shadow-lg md:rounded-b-lg'} `}>
        <div className="overflow-hidden ">
          ...
          <ButtonUI clasName={ `${isRow && `col-span-4 md:col-span-3 ${CountActiveProductBasket > 0 ? ' md:!col-span-2' :'col-span-3'}` }  ${CountActiveProductBasket > 0 ? ' md:col-span-2' :'col-span-3'}    `} btnCard href={product?.link} text={t('btn.more')}/>
          </div>
        </div>
      </div>
    </div>
