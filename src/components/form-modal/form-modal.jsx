...
const FormModal = ({isOpen}) => {
    ...
    return (
        ...
        <motion.div initial="hidden"
               animate="visible"
               exit="hidden"
               variants={overlayVariants}  className={'w-screen flex flex-col items-center justify-center  h-screen fixed top-0 left-0 right-0 bottom-0 bg-currentBlue/60 z-[99999]'} >
            <motion.div initial={{ y: "100vh" ,scaleY: 0 }}
                        animate={{ y: 0 , scaleY: 1 }}
                        exit={{  y: "100vh" , scaleY:0 }}
                        transition={{ duration: 0.4 }} className={'bg-white rounded-lg p-5 min-w-[300px] md:p-10 md:w-[500px] flex flex-col items-center gap-5'} >
